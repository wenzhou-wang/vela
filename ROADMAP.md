...
- ⬜ **Skeletal animation & skinning**
  - Parse glTF `animations` (samplers/channels) and `skins` (inverse bind matrices).
  - `AnimationClip` / `AnimationMixer` with linear + cubic-spline interpolation;
    quaternion tracks use slerp.
  - GPU skinning: upload joint matrices to a storage buffer, add a skinning vertex
    stage variant (JOINTS_0/WEIGHTS_0 attributes) — pipeline key gains a `skinned` bit.
